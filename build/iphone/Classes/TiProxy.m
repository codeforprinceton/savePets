...
//Rare exceptions to indicate a bug in the _save code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_save = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_save]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.save.module"
